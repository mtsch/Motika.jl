...
```julia
julia> @harvest function energies(meta, data)
    id = "test"
    Ntw = meta.n_target
    len = nrow(data)
...
julia> @harvest(function(meta, data)
    id = "test"
    Ntw = meta.n_target
    len = nrow(data)
    μ_shift, σ_shift = mean_and_se(data.shift)
...
"""
    harvest(f, dir; skip=0)

Harvest a run by applying the function `f` to every file in the directory `dir`.  `f`
should take a `DataFrame` as the only argument. The first `skip` rows of the `DataFrames`
are skipped.
